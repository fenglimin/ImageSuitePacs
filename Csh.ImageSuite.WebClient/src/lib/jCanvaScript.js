...
(function (window, undefined) {
    ...
    var log = function (msg) {//TO DO: just test
        if ($("#ulLog").length > 0) {
            $("#ulLog").prepend('<li>' + msg + '</li>');
            if ($("#ulLog li").length > 10) {
                $("#ulLog li:last-child").remove();
            }
        } else {
            console.info(msg);
        }
    ...
    function isPointInPath(object, x, y) {
        ...
        if (ctx.isPointInPath === undefined || object._img !== undefined || object._imgData !== undefined || object._proto == 'text') {
            ...
            if (rectangle.x <= poorPoint.x && rectangle.y <= poorPoint.y && (rectangle.x + rectangle.width) >= poorPoint.x && (rectangle.y + rectangle.height) >= poorPoint.y) {
                //log('in point, proto:' + object._proto);
                return point;
            }
        }
        else if (object._proto == 'line') {
            var poorPoint = transformPoint(x, y, multiplyM(object.matrix(), layer.matrix()));
    ...
    proto.text = function () {
        ...
        this.splitLines = function (ctx) {
            var lines = this._lines = [],
                ...
                line = '',
                currentY = 0,
                words = this._string.split(' ');

            var m = this._font.match(/^(?:\d+).*?/);
            if (m) { fontSize = parseInt(m[0]) }
        ...
        this.draw = function (ctx) {
            if (this._maxWidth === false) {
                ...
                if (!this._splitedLines) {
                    this.splitLines(ctx);
                    this._splitedLines = true;
                }

                if (this._fill) {
                    for (var i = 0, len = this._lines.length; i < len; i++) {
                        ctx.fillText(this._lines[i].text, this._x, this._y + this._lines[i].height);
                    }
    ...
    function jcFrame(time) {
        canvases.forEach(function (canvas) {
            if (!canvas.isPaused)
                canvas.frame(time);
        });

        return requestAnimFrame(jcFrame);
    }
    ...
    jCanvaScript.canvas = function (idCanvas) {
        ...
        canvas.height = function (height) {
            ...
            this.optns.height = this.cnv.height = height;
                ...
                if (mm.object != false) {
                    ...
                    if (underMouse === mousemoveObject) {
                        ...
                        if (mousemoveLayer === underMouseLayer) {
                            ...
                            if (typeof mousemoveLayer._onmouseover === 'function') {
                                mousemoveLayer._onmouseover(point);
                            }
                        ...
                        if (underMouse) {
                            if (typeof underMouse._onmouseout === 'function') {
                                underMouse._onmouseout(point);
                            }
                            if (typeof underMouse.onmouseout === 'function') {
                                underMouse.onmouseout(point);
                            }
                        }
                        if (typeof mousemoveObject._onmouseover === 'function') {
                            mousemoveObject._onmouseover(point);
                        }
                ...
                mdCicle:
                    for (i = mdObjectsLength; i > -1; i--) {
                        ...
                        for (var j = 0; j < 2; j++) {
                            ...
                            var toBreak = false;
                            if (typeof mdObject._onmousedown == 'function')
                                if (mdObject._onmousedown({ x: mouseDown.x, y: mouseDown.y, event: mouseDown.event }) === false)
                                    toBreak = true;
                ...
                muCicle:
                    for (i = mouseUp.objects.length - 1; i > -1; i--) {
                        ...
                        for (j = 0; j < 2; j++) {
                            ...
                            var toBreak = false;
                            if (typeof muObject._onmouseup == 'function')
                                if (muObject._onmouseup({ x: mouseUp.x, y: mouseUp.y, event: mouseUp.event }) === false)
                                    toBreak = true;
                            if (typeof muObject.onmouseup == 'function')
                                if (muObject.onmouseup({ x: mouseUp.x, y: mouseUp.y, event: mouseUp.event }) === false)
                                    toBreak = true;
                            if (toBreak) break muCicle;
                        }
                    }
            ...
            if (dblClick.objects.length) {
                dcCicle:
                    for (i = dblClick.objects.length - 1; i > -1; i--) {
                        var mouseDblClickObjects = [dblClick.objects[i], objectLayer(dblClick.objects[i])], clickObject;
                        for (j = 0; j < 2; j++) {
                            clickObject = mouseDblClickObjects[j];
                            stopDrag(clickObject, dblClick, optns);
